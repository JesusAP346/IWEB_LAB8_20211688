...
<table border="1">
    ...
    <tr>
        ...
        <td><%=movie.getGeneroB().getNombre()%></td>
        <td><%=movie.getDuracion()%></td>
        <td><%=movie.getStreamingB().getNombreServicio()%></td>
        <td><%=movie.isPremioOscar()%></td>
        <td><a href="listaActores?idPelicula=<%= movie.getIdPelicula() %>">Ver Actores</a></td>
        <%
            if (1 == 1) {
        %>
        <td><a onclick="return confirm('¿Esta seguro de borrar?')" href="<%=request.getContextPath()%>/listaPeliculas?action=borrar&idPelicula=<%=movie.getIdPelicula()%>" class="button-link">Borrar</a></td>
        <%
            }
        %>
    </tr>
